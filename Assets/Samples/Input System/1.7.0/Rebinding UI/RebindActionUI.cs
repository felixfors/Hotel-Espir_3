...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        [SerializeField]
        private InputActionReference cancelActionInput;
        ...
        {
            var savedBinding = PlayerPrefs.GetString(m_Action.action.name + bindingId);
            if(!string.IsNullOrEmpty(savedBinding))
            {
                actionReference.action.actionMap.LoadBindingOverridesFromJson(savedBinding);
            }
        }
        public void DeleteActionBinding()
        {
            var savedBinding = PlayerPrefs.GetString(m_Action.action.name + bindingId);
            if (!string.IsNullOrEmpty(savedBinding))
            {
                PlayerPrefs.DeleteKey(m_Action.action.name + bindingId);
                ResetToDefault();
            }
        }
        private void Update()
        {
            if (ongoingRebind != null && Input.GetKeyDown(KeyCode.Escape))
            {
